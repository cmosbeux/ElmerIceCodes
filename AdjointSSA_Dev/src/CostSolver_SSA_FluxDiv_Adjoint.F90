...
!  
  ...
  real(kind=dp) :: Cost,Cost_S,Costb, Lambda, STOT
  ...
  Logical :: Firsttime=.true., SurfaceNotComputed = .true., Found,Parallel,stat,Gotit
  CHARACTER*10 :: date,temps

  save Firsttime,STOT,Parallel, SurfaceNotComputed 
  save SolverName,CostSolName,CostFile, Lambda
  save ElementNodes
...
!------------------------------------------------------------------------------
                ...
                !surface computing for adimensionnalisation
                STOT = STOT + S

        END DO 
        END IF !Surface computed OK
...
!------------------------------------------------------------------------------
          ...
          Cost=Cost+0.5*lambda*coeff*coeff*s/STOT
         
         ! compute the derivatives
          Costb=0.5*lambda*s*2.0*coeff/STOT
    ...
    !Make it "adimensionnal"
    !Cost = Cost/STOT
    
    SurfaceNotComputed = .false. 
    ...
    IF (Parallel) THEN
         ...
         IF (Solver % Matrix % ParMatrix % ParEnv % MyPE == 0) then
                 ...
                 write(12,'(e13.5,2x,e15.8,2x,e15.8)') TimeVar % Values(1),Cost_S,sqrt(2.0*Cost_S/lambda)
                 CLOSE(12)
         End if
   ELSE
            CostVar => VariableGet( Solver % Mesh % Variables, CostSolName )
            ...
            END IF
                    OPEN (12, FILE=CostFile,POSITION='APPEND')
                       write(12,'(e13.5,2x,e15.8,2x,e15.8)') TimeVar % Values(1),Cost, sqrt(2.0*Cost/lambda)
                    close(12)
   END IF
