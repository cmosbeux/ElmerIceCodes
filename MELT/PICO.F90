...
!------------------------------------------------------------------------------
  ...
  !  Melt => MeltVar % Values
    ...
    IF (Firsttime) THEN
      ...
      ALLOCATE(Zbox(boxmax,MaxBas), Abox(boxmax,MaxBas), Tbox(boxmax,MaxBas), Sbox(boxmax,MaxBas), Mbox(boxmax,MaxBas))
      ...
      IF (DIM .EQ. 3) THEN
        IF (MAXVAL(Boxnumber(BPerm(NodeIndexes(1:n))))==1 ) THEN
          ...
          totalmelt = totalmelt &
            ...
            & * SUM(localunity(NodeIndexes(1:n)))/SIZE(NodeIndexes(:))
        ENDIF
      ELSE 
