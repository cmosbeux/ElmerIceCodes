...
!------------------------------------------------------------------------------
!> Initialization of the main solver: DamageInitialization
!------------------------------------------------------------------------------
   SUBROUTINE DamageInitializationInit( Model,Solver,Timestep,TransientSimulation )
!------------------------------------------------------------------------------
     USE DefUtils
     IMPLICIT NONE
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
   ...
   END SUBROUTINE DamageInitializationInit
...
!------------------------------------------------------------------------------
     ...
     CALL Info( 'DamageInitialization', ' ', Level=4 )
     CALL Info( 'DamageInitialization', &
         '-------------------------------------', Level=4 )
     CALL Info( 'DamageInitialization', 'Solving for Damage increase (no transport)', Level=4 )
     CALL Info( 'DamageInitialization', &
         '-------------------------------------', Level=4 )
...
!------------------------------------------------------------------------------
     ...
     ELSE
       ...
       ELSE IF( Stabilize .AND. Bubbles ) THEN
         CALL Fatal('DamageInitialization','Choose either Bubbles or Stabilize!')
       END IF
     END IF
...
!------------------------------------------------------------------------------
       ...
       CALL Info( 'DamageInitialization','Bulk Assembly')
       ...
       DO t = 1, tmax
         ...
         IF ( RealTime() - at0 > 1.0 ) THEN
           ...
           CALL Info( 'DamageInitialization', Message, Level=5 )
           at0 = RealTime()
         END IF
...
!------------------------------------------------------------------------------
    Diffusivity = 0.0d0
    Density(1:n) = 1.0_dp !ListGetReal( Material,'Density',n,NodeIndexes )
