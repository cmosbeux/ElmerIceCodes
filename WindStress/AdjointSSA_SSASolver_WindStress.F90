...
! *  Modification : Cyrille Mosbeux (adding wind skin stress)
...
!------------------------------------------------------------------------------
  ...
  REAL(KIND=dp), ALLOCATABLE :: STIFF(:,:), LOAD(:), FORCE(:), &
           ...
           NodalZs(:), NodalZb(:),   &
           NodalU(:), NodalV(:), NodalBeta(:),LocalLinVelo(:), NodalWind(:,:)

  INTEGER :: iFriction
  REAL(KIND=dp) :: fm, icedrag, rhoair
  ...
  SAVE rhow,sealevel,rhoair,icedrag
  SAVE STIFF, LOAD, FORCE, AllocationsDone, DIM, SolverName, ElementNodes, NodalWind
...
!------------------------------------------------------------------------------
  ...
  IF ( (.NOT. AllocationsDone) .OR. Solver % Mesh % Changed  ) THEN
     ...
     IF (AllocationsDone) DEALLOCATE(FORCE, LOAD, STIFF, NodalGravity, &
                       ...
                       NodalZb, NodalZs,  NodalU, NodalV,NodalWind,  &
     ...
     ALLOCATE( FORCE(STDOFs*N), LOAD(N), STIFF(STDOFs*N,STDOFs*N), &
          ...
          NodalZb(N), NodalZs(N), NodalWind(2,N), &
...
!------------------------------------------------------------------------------
  ...
  DO t=1,Solver % NumberOfActiveElements
   ...
   END SELECT
     ...
     !Get wind density drag coefficient as constants
     icedrag = ListGetConstReal( Material, 'ice drag', Found )
     If (.NOT.Found) Then
            WRITE(Message,'(A)') 'Constant Ice Drag not found. &
                   &Setting to 2e-3_dp'
            CALL INFO(SolverName, Message, level=20)
            icedrag = 2e-3_dp
     End if

     rhoair = ListGetConstReal( Material, 'Air Density', Found )
     If (.NOT.Found) Then
            WRITE(Message,'(A)') 'Constant Air Density not found. &
                   &Setting to 1.3468e-21 (density at -20C) '
            CALL INFO(SolverName, Message, level=20)
            rhoair = 1.3468e-21_dp
     End if
     
     !Add wind in the two directions
     NodalWind = 0.0_dp
     NodalWind(1,1:n) = ListGetReal( Material, 'Wind-X', n, NodeIndexes, Found )
     IF (.NOT.Found) &
        CALL FATAL(SolverName,'Could not find Material prop. >Wind-X<')
     IF (STDOFs==2) THEN
        NodalWind(2,1:n) = ListGetReal( Material, 'Wind-Y', n, NodeIndexes, Found )
     END IF
     ...
     CALL LocalMatrixUVSSA (  STIFF, FORCE, Element, n, ElementNodes, NodalGravity, &
        ...
        NodalU, NodalV, NodalBeta,iFriction,fm,LocalLinVelo, cn, NodalWind, rhoair, icedrag, MinSRInv , STDOFs, Newton)
...
!------------------------------------------------------------------------------
  SUBROUTINE LocalMatrixUVSSA(  STIFF, FORCE, Element, n, Nodes, gravity, &
           ...
           LocalV, LocalBeta,iFriction,fm,LocalLinVelo, cm, LocalWind, dair, idrag, MinSRInv, STDOFs , Newton )
!------------------------------------------------------------------------------
    REAL(KIND=dp) :: STIFF(:,:), FORCE(:), gravity(:), Density(:), &
                     Viscosity(:), LocalZb(:), LocalZs(:), &
                     LocalU(:), LocalV(:) , LocalBeta(:), LocalLinVelo(:), LocalWind(:,:)
    INTEGER :: n, cp , STDOFs
    INTEGER :: iFriction
    REAL(KIND=dp) :: cm,fm, dair, idrag
...
!------------------------------------------------------------------------------
    ...
    REAL(KIND=dp) :: beta,Slip2,Velo(2),LinVelo,ub, WindDrag(2)
...
! Needed Integration Point value
   ...
   !-------------------------------------------------
   !Wind Drag
   WindDrag(1) = dair * idrag * SUM(Localwind(1,1:n) * Basis(1:n))**2
   WindDrag(2) = dair * idrag * SUM(Localwind(2,1:n) * Basis(1:n))**2
...
! In the non-linear case, effective viscosity       
       ...
       DO p=1,n
         ...
         FORCE((STDOFs)*(p-1)+i) =   FORCE((STDOFs)*(p-1)+i) - &   
            rho*g*h*gradS(i) * IP % s(t) * detJ * Basis(p) - WindDrag(i) * IP % s(t) * detJ * Basis(p) 
         END DO
